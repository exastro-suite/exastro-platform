        ...
        for row in organization_db_rows:
            ...
            with closing(db.connect_orgdb(organization_id)) as conn:
                with conn.cursor() as cursor:
                    ...
                    try:
                        globals.logger.info(f"[{self.step_count}/{self.step_max}] - modify column audit_log: organization_id:[{organization_id}]")
                        query = queries_db_organizations.MODIFY_COLUMN_AUDIT_LOG
                        globals.logger.debug(f'EXECUTE SQL:{query}')
                        cursor.execute(query)

                        globals.logger.info(f"[{self.step_count}/{self.step_max}] -- OK: modify column audit_log: organization_id:[{organization_id}")
                        ...
                        globals.logger.info(f"[{self.step_count}/{self.step_max}] -- NG: modify column audit_log: organization_id:[{organization_id}")
                        globals.logger.error(f"exception:{e.args}")
                        message_id = "500-90031"
                        message = multi_lang.get_text(message_id,
                                                      "organization_db modify column audit_log failed. organization_id:[{0}]",
                                                      organization_id)
                        raise common.InternalErrorException(message_id=message_id, message=message)
