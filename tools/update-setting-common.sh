...
if [ ! -z "${PARAM_JSON_FILE}" ]; then
    if [ ! -f "${PARAM_JSON_FILE}" ]; then
        echo "Error: not found update-setting json file : ${PARAM_JSON_FILE}"
        exit 1
    fi
fi
...
echo
echo "Please enter the key to set the setting"
echo
read -p "key : " SETTING_KEY
...
curl ${CURL_OPT} -X PUT \
    ...
    -w '%{http_code}\n' \
    "${CONF_BASE_URL}/api/platform/settings/common/${SETTING_KEY}" > "${TEMPFILE_API_CODE}"
